(function(){
    ...
    UM.registerWidget(widgetName, {
        ...
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=video_url%>video.css\" />" +
            ...
            "<table>" +
            "<tr><td><label for=\"eduiVideoWidth\"><%=lang_videoW%></label></td><td><input class=\"edui-video-txt\" id=\"eduiVideoWidth\" value=\"100%\" type=\"text\"/></td></tr>" +
            "<tr><td><label for=\"eduiVideoHeight\"><%=lang_videoH%></label></td><td><input class=\"edui-video-txt\" id=\"eduiVideoHeight\" value=\"200\" type=\"text\"/></td></tr>" +
            "</table>" +
            "</fieldset>" +
            "<fieldset>" +
        ...
        initController: function( lang ){
            ...
            var me = this,
                vedio_iframe = me.editor.selection.getRange().getClosedNode(),
                url;

            me.createAlignButton( ["eduiVideoFloat"] );

            //编辑视频时初始化相关信息
            if(vedio_iframe && vedio_iframe.className == "edui-faked-video"){
                $("#eduiVideoUrl", me.$widget)[0].value = url = vedio_iframe.getAttribute("src");
                $("#eduiVideoWidth", me.$widget)[0].value = vedio_iframe.width;
                $("#eduiVideoHeight", me.$widget)[0].value = vedio_iframe.height;
                var align = domUtils.getComputedStyle(vedio_iframe, "float"),
                    parentAlign = domUtils.getComputedStyle(vedio_iframe.parentNode, "text-align");
        ...
        createPreviewVideo: function(url){
            ...
            if ( ! /(youku\.com)/.test(conUrl) ){
                $("#eduiVideoPreview", me.$widget).html( lang.urlError );
                return;
            }
            $("#eduiVideoPreview", me.$widget)[0].innerHTML = '<iframe class="edui-faked-video" src="' + conUrl + '" width="' + 420  + '" height="' + 280  + '" frameborder="0" allowfullscreen></iframe>'
        },
        /**
         * 将单个视频信息插入编辑器中
         */
        insertSingle: function(){
            ...
            var me = this,
                width = $("#eduiVideoWidth", me.$widget)[0] || '100%',
                height = $("#eduiVideoHeight", me.$widget)[0] || '200',
        ...
        //     return url;
        // },
        // http://v.youku.com/v_show/id_XNzYxNzQ1MDAw.html?f=22585202&ev=1
        convert_url: function(url){
          var newUrl, youku_url_prefix = 'http://player.youku.com/embed/', youku_video_id;
          if ( !url ) return '';
            url.match(/id\_([\w\d]*)\.html/);
            youku_video_id = RegExp.$1;
            newUrl = youku_url_prefix + youku_video_id
          return newUrl;
        },
        /**
